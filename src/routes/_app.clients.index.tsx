                ...
                <TableCell>
                  <div className="flex items-center gap-2">
                    <Avatar className="h-8 w-8">
                      <AvatarFallback className="text-xs bg-muted">{initials(c.person.name)}</AvatarFallback>
                    </Avatar>
                    <span className="font-medium">{c.person.name}</span>
                  </div>
                </TableCell>
                <TableCell className="font-mono text-xs">{c.person.cpf ?? c.person.cnpj}</TableCell>
                  ...
                  <div className="flex gap-1">
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-48">
                        ...
                        <DropdownMenuItem onClick={() => navigate({ to: "/clients/$id", params: { id: String(c.id) } })}>
                          <Eye className="h-4 w-4" /> Ver detalhes
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => navigate({ to: "/clients/edit/$id", params: { id: String(c.id) } })}>
                          <Pencil className="h-4 w-4" /> Editar
                        </DropdownMenuItem>
                        <DropdownMenuSeparator />
                        <DropdownMenuItem
                          className="text-destructive focus:text-destructive"
                          onClick={() => toast.success(`Cliente ${c.person.name} removido`)}
                        >
